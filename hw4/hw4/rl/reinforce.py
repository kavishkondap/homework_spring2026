...
class Reinforce(RLAlgorithm):
    ...
    def update(
        ...
        for mb in iter_minibatches(
            ...
            new_logp = compute_per_token_logprobs(model, mb.input_ids, mb.attention_mask) #for all tokens except the first so (B, L-1)
            seq_logp_i = masked_mean_per_row(new_logp, mask) #(B,)
            pg_loss = -torch.mean(seq_logp_i*adv)
