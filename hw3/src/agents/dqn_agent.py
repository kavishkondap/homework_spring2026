...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.0) -> int:
        ...
        if torch.rand(1).item() < epsilon:
            action = torch.randint(self.num_actions, size=(1, 1))
        else:
            action = torch.argmax(self.critic(observation), dim=1, keepdim=True)
        # ENDTODO
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_qa_values = self.critic(next_obs) #(B, num_actions)

            if self.use_double_q:
                # TODO(Section 2.5): implement double-Q target action selection
                next_action = torch.argmax(self.target_critic(next_obs), dim=1)
            else:
                next_action = torch.argmax(next_qa_values, dim=1) #(B, )

            next_q_values = next_qa_values[torch.arange(batch_size), next_action] #check if this is correct...
            assert next_q_values.shape == (batch_size,), next_q_values.shape

            target_values = reward + self.discount * (1-done.int()) * next_q_values #if trajectory is done, then don't care abt the next obs bc we are done
        ...
        q_values = qa_values[torch.arange(batch_size), action]
        loss = (q_values - target_values).pow(2).mean() #check this...
        # ENDTODO
